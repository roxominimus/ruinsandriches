			...
			protected override void OnTarget( Mobile from, object o )
			{
				if ( o is Mobile && o is BaseCreature )
				{
					...
					else if ( bc.ControlSlots == 666 )
					{
						...
						{
							Effects.SendLocationParticles( EffectItem.Create( m.Location, m.Map, EffectItem.DefaultDuration ), 0x3728, 8, 20, Server.Misc.PlayerSettings.GetMySpellHue( true, from, 0 ), 0, 5042, 0 );
						...
						{
							m.FixedEffect( 0x3779, 10, 20, Server.Misc.PlayerSettings.GetMySpellHue( true, from, 0 ), 0 );
