		...
		private class InternalTimer : Timer
		{
			...
			protected override void OnTick()
			{
				if ( m_Attacker.HarmfulCheck( m_Defender ) )
				{
					...
					{
						m_Target.FixedParticles( 0x36BD, 20, 10, 5044, Server.Misc.PlayerSettings.GetMySpellHue( true, m_Attacker, 0 ), 0, EffectLayer.Head );
					}
					else
					{
						Effects.SendLocationEffect( m_Target.Location, m_Target.Map, 0x3822, 60, 10, Server.Misc.PlayerSettings.GetMySpellHue( true, m_Attacker, 0 ), 0 );
					}
