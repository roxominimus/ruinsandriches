	...
	public class ManaDrainSpell : MagerySpell
	{
		...
		private void AosDelay_Callback( object state )
		{
			...
			if ( m.Alive && !m.IsDeadBondedPet )
			{
				...
				m.FixedEffect( 0x3779, 10, 25, Server.Misc.PlayerSettings.GetMySpellHue( true, Caster, 0 ), 0 );
				m.PlaySound( 0x28E );
			}
		...
		public void Target( Mobile m )
		{
			...
			else if ( CheckHSequence( m ) )
			{
				...
				if ( Core.AOS )
				{
					...
					m.FixedParticles( 0x3789, 10, 25, 5032, Server.Misc.PlayerSettings.GetMySpellHue( true, Caster, 0 ), 0, EffectLayer.Head );
					m.PlaySound( 0x1F8 );
				...
				else
				{
					...
					m.FixedParticles( 0x374A, 10, 15, 5032, Server.Misc.PlayerSettings.GetMySpellHue( true, Caster, 0 ), 0, EffectLayer.Head );
					m.PlaySound( 0x1F8 );
				}
