		...
		public void Target( IPoint3D p )
		{
			...
			else if ( CheckSequence() )
			{
				...
				for ( int i = 0; i < targets.Count; ++i )
				{
					...
					{
						Effects.SendLocationParticles( EffectItem.Create( m.Location, m.Map, EffectItem.DefaultDuration ), 0x3728, 8, 20, Server.Misc.PlayerSettings.GetMySpellHue( true, Caster, 0 ), 0, 5042, 0 );
					...
					else
					{
						...
						m.FixedEffect( 0x3779, 10, 20, Server.Misc.PlayerSettings.GetMySpellHue( true, Caster, 0 ), 0 );
					}
				}
			}
