		...
		public void Target( TrapableContainer item )
		{
			...
			else if ( CheckSequence() )
			{
				...
				if ( (int)(Caster.Skills[SkillName.Magery].Value ) > nTrapLevel )
				{
					...
					Effects.SendLocationParticles( EffectItem.Create( loc, item.Map, EffectItem.DefaultDuration ), 0x376A, 9, 32, Server.Misc.PlayerSettings.GetMySpellHue( true, Caster, 0 ), 0, 5015, 0 );
			...
			protected override void OnTarget( Mobile from, object o )
			{
				...
				else if ( from == o )
				{
					if ( m_Owner.CheckSequence() )
					{
						...
						from.FixedParticles( 0x376A, 9, 32, 5008, Server.Misc.PlayerSettings.GetMySpellHue( true, from, 0 ), 0, EffectLayer.Waist );
