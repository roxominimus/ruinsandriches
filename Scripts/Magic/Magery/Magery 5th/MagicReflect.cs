	...
	public class MagicReflectSpell : MagerySpell
	{
		...
		public override void OnCast()
		{
			...
			else if ( CheckSequence() )
			{
				...
				{
					int value = (int)( ( Caster.Skills[SkillName.Magery].Value + Caster.Skills[SkillName.Psychology].Value ) / 4 );
					Caster.MagicDamageAbsorb = value;
					Item diamond = Caster.Backpack.FindItemByType( typeof ( Diamond ) );
					if ( diamond != null ){ diamond.Consume(); }
					Caster.PlaySound( 0x1ED );
					Caster.FixedParticles( 0x375A, 10, 15, 5037, Server.Misc.PlayerSettings.GetMySpellHue( true, Caster, 0 ), 0, EffectLayer.Waist );
				}
				else
				{
