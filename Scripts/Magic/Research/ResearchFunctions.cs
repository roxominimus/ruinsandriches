...
namespace Server.Misc
{
    class Research
    {
		...
		public static void FindLocation( Mobile m, int level, string category, ResearchBag bag ) ////////////////////////////////////////////////////
		{
			...
			{
				if ( Server.Misc.Worlds.GetMyWorld( target.Map, target.Location, target.X, target.Y ) == searchLocation && Server.Misc.Worlds.GetRegionName( target.Map, target.Location ) != oldDungeon )
				{
					targets.Add( target );
					aCount++;
				}
