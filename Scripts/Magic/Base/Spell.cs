...
namespace Server.Spells
{
	public abstract class Spell : ISpell
	{
		...
		public static bool CanCastSpell( Mobile m, ISpell s )
		{
			...
			{
				if ( Worlds.GetMyWorld( m.Map, m.Location, m.X, m.Y ) == "the Underworld" && ( s is MagerySpell || s is ResearchSpell || s is ElementalSpell ) && Utility.RandomMinMax(1,3) != 1 )
				{
		...
		public virtual bool CheckSequence()
		{
			...
			else if ( m_Scroll != null && !(m_Scroll is RuneBag) && !(m_Scroll is BaseMagicStaff) && !(m_Scroll is Runebook) && (m_Scroll.Amount <= 0 || m_Scroll.Deleted || m_Scroll.RootParent != m_Caster ) )
			{
				DoFizzle();
			}
