		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			if ( !e.Handled )
			{
				...
				else if ( m.Region.IsPartOf( "Dungeon Rock" ) ) // EXODUS - FORGE OF VIRTUE
				{
					if ( m.Backpack.FindItemByType( typeof ( DarkCoreExodus ) ) != null )
					{
						...
						foreach ( Item enchant in m.GetItemsInRange( 20 ) )
						{
							...
							{
								if ( !Server.Misc.Arty.canEnchant( enchant ) && enchant.X>=704 && enchant.Y>=2208 && enchant.X<=705 && enchant.Y<=2209 )
								{
									canProcess = false;
								...
								if ( !( enchant is BaseWeapon || enchant is BaseArmor || enchant is BaseJewel || enchant is BaseClothing ) && enchant.X>=704 && enchant.Y>=2208 && enchant.X<=705 && enchant.Y<=2209 )
								{
									nonCount++;
									if ( nonCount > 0 )
										nonProcess = false;
						...
						else
						{
							...
							foreach ( Item enchant in m.GetItemsInRange( 20 ) )
							{
								if ( enchant.ItemID != 5703 )
								{
									if ( Server.Misc.Arty.canEnchant( enchant ) && enchant.X>=704 && enchant.Y>=2208 && enchant.X<=705 && enchant.Y<=2209 && Change == 0 )
									{
										int min = 50;
										int max = 200;
				...
				else if ( m.Map == Map.SavagedEmpire ) // POISON FORGE
				{
					foreach ( Item enchant in m.GetItemsInRange( 20 ) )
					{
						if ( Server.Misc.Arty.canEnchant( enchant ) && enchant.X>=1064 && enchant.Y>=1167 && enchant.X<=1067 && enchant.Y<=1169 )
						{
							if ( enchant is BaseWeapon )
							{
								BaseWeapon weapon = (BaseWeapon)enchant;
				...
				else if ( m.Map == Map.Lodor ) // COLD FORGE
				{
					foreach ( Item enchant in m.GetItemsInRange( 20 ) )
					{
						if ( Server.Misc.Arty.canEnchant( enchant ) && enchant.X>=3979 && enchant.Y>=736 && enchant.X<=3982 && enchant.Y<=738 )
						{
							if ( enchant is BaseWeapon )
							{
								BaseWeapon weapon = (BaseWeapon)enchant;
				...
				else if ( m.Map == Map.Sosaria ) // ENERGY FORGE
				{
					foreach ( Item enchant in m.GetItemsInRange( 20 ) )
					{
						if ( Server.Misc.Arty.canEnchant( enchant ) && enchant.X>=5331 && enchant.Y>=3203 && enchant.X<=5334 && enchant.Y<=3207 )
						{
							if ( enchant is BaseWeapon )
							{
								BaseWeapon weapon = (BaseWeapon)enchant;
				...
				else if ( m.Map == Map.SerpentIsland ) // FIRE FORGE
				{
					foreach ( Item enchant in m.GetItemsInRange( 20 ) )
					{
						if ( Server.Misc.Arty.canEnchant( enchant ) && enchant.X>=725 && enchant.Y>=1151 && enchant.X<=730 && enchant.Y<=1156 )
						{
							if ( enchant is BaseWeapon )
							{
								BaseWeapon weapon = (BaseWeapon)enchant;
