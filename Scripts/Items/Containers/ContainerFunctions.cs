    ...
    class ContainerFunctions
    {
		...
		public static void FillTheContainer( int level, LockableContainer box, Mobile opener )
		{
			...
			for ( int i = 0; i < ( filled ); ++i )
			{
				...
				{
					if ( 1 == Utility.Random( 4 ) ){ Item idropped = new ScrollClue(); box.DropItem( idropped ); }
					else { Item idropped = DungeonLoot.RandomLoreBooks(); box.DropItem( idropped ); }
				}
				if ( iRich > Utility.Random( 5000 ) && IgnoreThisFiller == 0 && opener != null && box is DungeonChest )
				{
					if ( opener.Skills[SkillName.Forensics].Value >= 30 ){ Item idropped = new FrankenJournalInBox(); box.DropItem( idropped ); }
				}
				...
				if ( iRich > Utility.Random( 10000 ) && IgnoreThisFiller == 0 && opener != null && Utility.RandomBool() )
				{
					int tMapLevel = (int)( level / 2 ); if ( tMapLevel < 1 ){ tMapLevel = 1; } else if ( tMapLevel > 6 ){ tMapLevel = 6; }
					box.DropItem( new TreasureMap( tMapLevel, opener.Map, opener.Location, opener.X, opener.Y ) );
				}
				if ( iRich > Utility.Random( 100 ) && IgnoreThisFiller == 0 )
				{
				...
				if ( iRich > Utility.Random( 1000 ) && IgnoreThisFiller != 2 )
				{
					...
					if ( idropped is UnusualDyes && Server.Misc.GetPlayerInfo.EvilPlay( opener ) ){ idropped.Hue = Utility.RandomEvilHue(); }
					...
					else if ( ( idropped is SmallHollowBook || idropped is LargeHollowBook ) && Utility.RandomBool() )
					{
						...
						{
							int buk = 1; if ( IgnoreThisFiller != 2 ){ buk = 0; }
							...
							if ( (level * 9) > Utility.Random( 100 ) )
							{
								if ( booking == 0 )
								{
									...
									{
										int mgsc = rich+3; if (mgsc > 63){ mgsc = Utility.RandomMinMax(56,63); }
										int elsc = (rich+3/2); if (elsc > 31){ elsc = Utility.RandomMinMax(28,31); }
										int scroll = Utility.RandomMinMax(1,5);
										if ( scroll > 2 ) { Item item = Loot.RandomScroll( 0, mgsc, SpellbookType.Regular ); iBook.DropItem( item ); }
										else if ( scroll == 2 ) { Item item = Loot.RandomScroll( 0, elsc, SpellbookType.Elementalism ); iBook.DropItem( item ); }
										else { Item item = Loot.RandomScroll( 0, 17, SpellbookType.Necromancer ); iBook.DropItem( item ); }
									}
								}
								else if ( booking == 1 )
								{
					...
					else if ( ((idropped is GiantBag) || (idropped is EnormousBag)) && (iRich+300 > Utility.Random(300)) )
					{
						string baggy = "Giant";
						...
						switch ( Utility.RandomMinMax( 0, 13 ) ) 
						{
							...
							case 2: 	idropped.Hue = MaterialInfo.GetMaterialColor( "serpent", "", 0 );			idropped.Name = "Serpent Leather " + baggy + " Bag";	break;
							case 3: 	idropped.Hue = MaterialInfo.GetMaterialColor( "unicorn skin", "", 0 );		idropped.Name = "Unicorn Skin " + baggy + " Bag";		break;
							case 4: 	idropped.Hue = MaterialInfo.GetMaterialColor( "nightmare skin", "", 0 );	idropped.Name = "Nightmare Skin " + baggy + " Bag";		break;
							case 5: 	idropped.Hue = MaterialInfo.GetMaterialColor( "serpent skin", "", 0 );		idropped.Name = "Serpent Skin " + baggy + " Bag";		break;
							case 6: 	idropped.Hue = MaterialInfo.GetMaterialColor( "troll skin", "", 0 );		idropped.Name = "Troll Skin " + baggy + " Bag";			break;
							case 7: 	idropped.Hue = MaterialInfo.GetMaterialColor( "necrotic", "", 0 );			idropped.Name = "Necrotic Leather " + baggy + " Bag";	break;
							case 8: 	idropped.Hue = MaterialInfo.GetMaterialColor( "volcanic", "", 0 );			idropped.Name = "Volcanic Leather " + baggy + " Bag";	break;
							case 9: 	idropped.Hue = MaterialInfo.GetMaterialColor( "frozen", "", 0 );			idropped.Name = "Frozen Leather " + baggy + " Bag";		break;
							case 10: 	idropped.Hue = MaterialInfo.GetMaterialColor( "goliath", "", 0 );			idropped.Name = "Goliath Leather " + baggy + " Bag";	break;
							case 11: 	idropped.Hue = MaterialInfo.GetMaterialColor( "draconic", "", 0 );			idropped.Name = "Draconic Leather " + baggy + " Bag";	break;
							case 12: 	idropped.Hue = MaterialInfo.GetMaterialColor( "hellish", "", 0 );			idropped.Name = "Hellish Leather " + baggy + " Bag";	break;
							case 13: 	idropped.Hue = MaterialInfo.GetMaterialColor( "dinosaur", "", 0 );			idropped.Name = "Dinosaur Leather " + baggy + " Bag";	break;
						}
					}
					box.DropItem( idropped );
				}
			...
			if ( iRich > Utility.Random( 100 ) && ( IgnoreThisFiller == 0 || IgnoreThisFiller == 2 ) )
			{
				...
				if ( MyServerSettings.GetUnidentifiedChance() >= Utility.RandomMinMax( 1, 100 ) )
				{
					...
					{
						if ( weapon is WizardStick ){ weapon.ItemID = Utility.RandomList( 0x0DF2, 0x0DF3, 0x0DF4, 0x0DF5, 0x269D, 0x269E, 0x26BC, 0x26C6, 0x639D, 0x639E, 0x639F, 0x63A0 ); itemID.ItemID = weapon.ItemID; }
						else { weapon.ItemID = Utility.RandomList( 0xE81, 0x13F8, 0xDF1, 0x2D25, 0xE89, 0x0908 ); itemID.ItemID = weapon.ItemID; }
					}
					else if ( weapon is GiftThrowingGloves || weapon is GiftPugilistGloves || weapon is LevelThrowingGloves || weapon is LevelPugilistGloves || weapon is ThrowingGloves || weapon is PugilistGloves || weapon is PugilistGlove || weapon is BaseWhip ){ itemID.VendorCanID = "Leatherworker"; }
			...
			for ( int i = 0; i < ( filled ); i++ )
			{
				if ( iRich > Utility.Random( 100 ) )
				{
					if ( Utility.RandomMinMax( 0, 4 ) == 1 )
					{
						...
						if ( Server.Misc.IntelligentAction.TestForReagent( opener, "mixologist" ) && Utility.RandomBool() )
							{ item.Delete(); item = Loot.RandomMixerReagent(); }

						if ( Server.Misc.IntelligentAction.TestForReagent( opener, "necromancer" ) && Utility.RandomBool() )
							{ item.Delete(); item = Loot.RandomNecromancyReagent(); }

						if ( Server.Misc.IntelligentAction.TestForReagent( opener, "wizard" ) && Utility.RandomBool() )
							{ item.Delete(); item = Loot.RandomReagent(); }

						if ( Server.Misc.IntelligentAction.TestForReagent( opener, "undertaker" ) && Utility.RandomBool() )
							{ item.Delete(); item = Loot.RandomWitchReagent(); }

						if ( Server.Misc.IntelligentAction.TestForReagent( opener, "druid" ) && Utility.RandomBool() )
							{ item.Delete(); item = Loot.RandomDruidReagent(); }
			...
			for ( int i = 0; i < fills; ++i )
			{
				if ( (level * 9) > Utility.Random( 100 ) && IgnoreThisFiller != 2 )
				{
					...
					else
					{
						...
						int elsc = (rich+3/2); if (elsc > 31){ elsc = Utility.RandomMinMax(28,31); }
						int scroll = Utility.RandomMinMax(1,5);
						if ( scroll > 2 ) { Item item = Loot.RandomScroll( 0, mgsc, SpellbookType.Regular ); }
						else if ( scroll == 2 ) { Item item = Loot.RandomScroll( 0, elsc, SpellbookType.Elementalism ); }
						else { Item item = Loot.RandomScroll( 0, 17, SpellbookType.Necromancer ); }
					}
				}
			}
		}
		...
		public static int BuildContainer( LockableContainer box, int thisHue, int thisItem, int thisGump, int thisDesign )
		{
			...
			if ( myBox == 1 )
			{
				...
				{
					case 0:		box.Name = "Oak Wood Chest";			hue = "oak";									break;
					case 1:		box.Name = "Ash Wood Chest";			hue = "ash";									break;
					case 2:		box.Name = "Cherry Wood Chest";			hue = "cherry";									break;
					case 3:		box.Name = "Walnut Wood Chest";			hue = "walnut";									break;
					case 4:		box.Name = "Golden Oak Wood Chest";		hue = "golden oak";								break;
					case 5:		box.Name = "Ebony Wood Chest";			hue = "ebony";									break;
					case 6:		box.Name = "Hickory Wood Chest";		hue = "hickory";								break;
					case 7:		box.Name = "Pine Wood Chest";			hue = "pine";									break;
					case 8:		box.Name = "Rosewood Chest";			hue = "rosewood";								break;
					case 9:		box.Name = "Mahogany Wood Chest";		hue = "mahogany";								break;
					case 10:	box.Name = "Drift Wood Chest";			hue = "driftwood";								break;
					case 11:	box.Name = "Worn Wooden Chest";			box.Hue = Utility.RandomMinMax( 2413, 2430 );	break;
				}
			...
			else if ( myBox == 7 )
			{
				...
				else if ( box.ItemID == 0x1248 || box.ItemID == 0x1264 || box.ItemID == 0x5777 || box.ItemID == 0x5776 ){ packName = "Sack"; packAdj = "Large "; }
		...
		public static void RelicValueIncrease( int level, Item item )
		{
			...
			else if ( item is DDRelicBearRugsAddonDeed){ DDRelicBearRugsAddonDeed relic = (DDRelicBearRugsAddonDeed)item; relic.RelicGoldValue = (int)(relic.RelicGoldValue * IncreaseValue) + relic.RelicGoldValue; }
		...
		{
			if ( item is BaseWeapon && item.Hue == 0 && Server.Misc.MaterialInfo.IsMetalItem( item ) && GetPlayerInfo.LuckyPlayer( from.Luck ) && 1 == Utility.Random( 1000 ) )
			{
				((BaseWeapon)item).Resource = CraftResource.None;
				switch( Utility.RandomMinMax( 0, 14 ) )
				{
					case 0:		MorphingItem.MorphMyItem( item, "IGNORED", "Caddellite", "IGNORED", MorphingTemplates.TemplateCaddellite("weapons") );	break;
					case 1:		MorphingItem.MorphMyItem( item, "IGNORED", "Emerald", "IGNORED", MorphingTemplates.TemplateEmerald("weapons") );		break;
					case 2:		MorphingItem.MorphMyItem( item, "IGNORED", "Garnet", "IGNORED", MorphingTemplates.TemplateGarnet("weapons") );			break;
					case 3:		MorphingItem.MorphMyItem( item, "IGNORED", "Ice", "IGNORED", MorphingTemplates.TemplateIce("weapons") );				break;
					case 4:		MorphingItem.MorphMyItem( item, "IGNORED", "Jade", "IGNORED", MorphingTemplates.TemplateJade("weapons") );				break;
					case 5:		MorphingItem.MorphMyItem( item, "IGNORED", "Marble", "IGNORED", MorphingTemplates.TemplateMarble("weapons") );			break;
					case 6:		MorphingItem.MorphMyItem( item, "IGNORED", "Onyx", "IGNORED", MorphingTemplates.TemplateOnyx("weapons") );				break;
					case 7:		MorphingItem.MorphMyItem( item, "IGNORED", "Quartz", "IGNORED", MorphingTemplates.TemplateQuartz("weapons") );			break;
					case 8:		MorphingItem.MorphMyItem( item, "IGNORED", "Ruby", "IGNORED", MorphingTemplates.TemplateRuby("weapons") );				break;
					case 9:		MorphingItem.MorphMyItem( item, "IGNORED", "Sapphire", "IGNORED", MorphingTemplates.TemplateSapphire("weapons") );		break;
					case 10:	MorphingItem.MorphMyItem( item, "IGNORED", "Silver", "IGNORED", MorphingTemplates.TemplateSilver("weapons") );			break;
					case 11:	MorphingItem.MorphMyItem( item, "IGNORED", "Spinel", "IGNORED", MorphingTemplates.TemplateSpinel("weapons") );			break;
					case 12:	MorphingItem.MorphMyItem( item, "IGNORED", "StarRuby", "IGNORED", MorphingTemplates.TemplateStarRuby("weapons") );		break;
					case 13:	MorphingItem.MorphMyItem( item, "IGNORED", "Topaz", "IGNORED", MorphingTemplates.TemplateTopaz("weapons") );			break;
					case 14:	MorphingItem.MorphMyItem( item, "IGNORED", "Amethyst", "IGNORED", MorphingTemplates.TemplateAmethyst("weapons") );		break;
				}
			}
			else if ( item is BaseArmor && item.Hue == 0 && Server.Misc.MaterialInfo.IsMetalItem( item ) && GetPlayerInfo.LuckyPlayer( from.Luck ) && 1 == Utility.Random( 1000 ) )
			{
				((BaseArmor)item).Resource = CraftResource.None;
				switch( Utility.RandomMinMax( 0, 14 ) )
				{
					case 0:		MorphingItem.MorphMyItem( item, "IGNORED", "Caddellite", "IGNORED", MorphingTemplates.TemplateCaddellite("armors") );	break;
					case 1:		MorphingItem.MorphMyItem( item, "IGNORED", "Emerald", "IGNORED", MorphingTemplates.TemplateEmerald("armors") );			break;
					case 2:		MorphingItem.MorphMyItem( item, "IGNORED", "Garnet", "IGNORED", MorphingTemplates.TemplateGarnet("armors") );			break;
					case 3:		MorphingItem.MorphMyItem( item, "IGNORED", "Ice", "IGNORED", MorphingTemplates.TemplateIce("armors") );					break;
					case 4:		MorphingItem.MorphMyItem( item, "IGNORED", "Jade", "IGNORED", MorphingTemplates.TemplateJade("armors") );				break;
					case 5:		MorphingItem.MorphMyItem( item, "IGNORED", "Marble", "IGNORED", MorphingTemplates.TemplateMarble("armors") );			break;
					case 6:		MorphingItem.MorphMyItem( item, "IGNORED", "Onyx", "IGNORED", MorphingTemplates.TemplateOnyx("armors") );				break;
					case 7:		MorphingItem.MorphMyItem( item, "IGNORED", "Quartz", "IGNORED", MorphingTemplates.TemplateQuartz("armors") );			break;
					case 8:		MorphingItem.MorphMyItem( item, "IGNORED", "Ruby", "IGNORED", MorphingTemplates.TemplateRuby("armors") );				break;
					case 9:		MorphingItem.MorphMyItem( item, "IGNORED", "Sapphire", "IGNORED", MorphingTemplates.TemplateSapphire("armors") );		break;
					case 10:	MorphingItem.MorphMyItem( item, "IGNORED", "Silver", "IGNORED", MorphingTemplates.TemplateSilver("armors") );			break;
					case 11:	MorphingItem.MorphMyItem( item, "IGNORED", "Spinel", "IGNORED", MorphingTemplates.TemplateSpinel("armors") );			break;
					case 12:	MorphingItem.MorphMyItem( item, "IGNORED", "StarRuby", "IGNORED", MorphingTemplates.TemplateStarRuby("armors") );		break;
					case 13:	MorphingItem.MorphMyItem( item, "IGNORED", "Topaz", "IGNORED", MorphingTemplates.TemplateTopaz("armors") );				break;
					case 14:	MorphingItem.MorphMyItem( item, "IGNORED", "Amethyst", "IGNORED", MorphingTemplates.TemplateAmethyst("armors") );		break;
				}
			}
		}
			...
			if ( item != null && canMod )
			{
				...
				else if ( item is BaseInstrument )
				{
					...
					else
					{
						switch ( instr.Resource )
						{
							...
							case CraftResource.PetrifiedTree: cHue = MaterialInfo.GetMaterialColor( "petrified", "", 0 ); cUse = 220; break;
				...
				if ( !plain && MyServerSettings.GetUnidentifiedChance() >= Utility.RandomMinMax( 1, 100 ) )
				{
					...
					else
					{
						...
						else if ( item is BaseWhip || item is ThrowingGloves || item is PugilistGloves || item is PugilistGlove ){ itemID.VendorCanID = "Leatherworker"; }
		...
		public static void GiveRandomItem( Mobile from )
		{
			...
			else if ( found == 7 )
			{
				...
				{
					int scroll = Utility.RandomMinMax(1,5);
					if ( scroll > 2 ) { loot = Loot.RandomScroll( 0, 63, SpellbookType.Regular ); }
					else if ( scroll == 2 ) { loot = Loot.RandomScroll( 0, 31, SpellbookType.Elementalism ); }
					else { loot = Loot.RandomScroll( 0, 17, SpellbookType.Necromancer ); }
				}
			}
			else if ( found == 8 )
			{
				if ( Utility.RandomMinMax( 0, 4 ) == 1 )
				{
					...
					if ( Server.Misc.IntelligentAction.TestForReagent( from, "undertaker" ) && Utility.RandomBool() )
						{ loot.Delete(); loot = Loot.RandomWitchReagent(); }

					if ( Server.Misc.IntelligentAction.TestForReagent( from, "druid" ) && Utility.RandomBool() )
						{ loot.Delete(); loot = Loot.RandomDruidReagent(); }
