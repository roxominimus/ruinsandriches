...
namespace Server.Items
{
	public class TreasureMap : MapItem
	{
		...
		private static Type[][] m_DefaultSpawns = new Type[][]
		{
			...
			new Type[]{ typeof( DreadSpider ), typeof( LichLord ), typeof( ClassicDaemon ), typeof( ElderGazer ), typeof( OgreLord ) },
			new Type[]{ typeof( LichLord ), typeof( ClassicDaemon ), typeof( ElderGazer ), typeof( PoisonElemental ), typeof( BloodElemental ) },
			new Type[]{ typeof( AncientWyrm ), typeof( ClassicBalron ), typeof( BloodElemental ), typeof( PoisonElemental ), typeof( Titan ), typeof( Dragon ) },
            new Type[]{ typeof( AncientWyrm ), typeof( ClassicBalron ), typeof( BloodElemental ), typeof( PoisonElemental ), typeof( ElderTitan ), typeof( Dragon ) }
		};
		...
		public TreasureMap( int level, Map map, Point3D location, int x, int y )
		{
			...
			else if ( world == "the Island of Umber Veil" ){ map = Map.Sosaria; }
			else if ( world == "the Bottle World of Kuldar" ){ map = Map.Sosaria; }
			...
			else if ( world == "the Land of Ambrosia" ){ map = Map.Sosaria; }
			else if ( world == "the World of Atlantis" ){ map = Map.Sosaria; }
			else { world = "the Land of Sosaria"; map = Map.Sosaria; }
