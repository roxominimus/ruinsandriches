            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is UnknownReagent && from.Backpack != null)
                {
                    ...
                    if (pack.ConsumeTotal(typeof(Gold), toConsume))
                    {
						...
						else if ( reagentType == 13 ){ from.AddToBackpack( new BitterRoot( RegCount ) ); reagentName = "bitter root"; }
						else if ( reagentType == 14 ){ from.AddToBackpack( new BlackSand( RegCount ) ); reagentName = "black sand"; }
						else if ( reagentType == 15 ){ from.AddToBackpack( new BloodRose( RegCount ) ); reagentName = "blood rose"; }
						else if ( reagentType == 16 ){ from.AddToBackpack( new DriedToad( RegCount ) ); reagentName = "dried toad"; }
						else if ( reagentType == 17 ){ from.AddToBackpack( new Maggot( RegCount ) ); reagentName = "maggot"; }
						else if ( reagentType == 18 ){ from.AddToBackpack( new MummyWrap( RegCount ) ); reagentName = "mummy wrap"; }
						else if ( reagentType == 19 ){ from.AddToBackpack( new VioletFungus( RegCount ) ); reagentName = "violet fungus"; }
						else if ( reagentType == 20 ){ from.AddToBackpack( new WerewolfClaw( RegCount ) ); reagentName = "werewolf claw"; }
						else if ( reagentType == 21 ){ from.AddToBackpack( new Wolfsbane( RegCount ) ); reagentName = "wolfsbane"; }
						else if ( reagentType == 22 ){ from.AddToBackpack( new SackFlour() ); RegCount=1; reagentName = "regular flour"; }
						else if ( reagentType == 23 ){ from.AddToBackpack( new FertileDirt() ); RegCount=1; reagentName = "plain dirt"; }
