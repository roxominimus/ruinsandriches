            ...
            protected override void OnTarget(Mobile from, object targeted)
			{
					...
					) && from.Backpack != null )
                {
                    ...
                    if (pack.ConsumeTotal(typeof(Gold), toConsume))
                    {
						...
						if ( targeted is BaseShoes || targeted is MagicBoots || targeted is LeatherSandals || targeted is LeatherShoes || targeted is HikingBoots || targeted is LeatherBoots || targeted is LeatherThighBoots || targeted is LeatherSoftBoots )
						{
							ba.ItemID = 0x4C27;
							ba.Name = adj + cls + "shoes";
						}
						else if ( targeted is BaseHat || targeted is MagicHat || targeted is LeatherCap || targeted is ShinobiCowl || targeted is ShinobiHood || targeted is ShinobiMask )
						{
							if ( ba.ItemID == 0x171C ){ ba.ItemID = 0x4C15; } else { ba.ItemID = 0x171C; }
							ba.Name = adj + cls + "hat";
						}
						else if ( targeted is ShinobiRobe || targeted is LeatherRobe || targeted is MagicRobe || targeted is BaseOuterTorso )
						{
