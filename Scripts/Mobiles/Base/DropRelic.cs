			...
			if ( killer != null && c != null && !bc.IsStabled && !bc.Controlled && !bc.IsBonded )
			{
				...
				if ( killer is PlayerMobile )
				{
					...
					if ( GetPlayerInfo.LuckyKiller( killer.Luck ) && Utility.RandomMinMax( 1, 20 ) == 1 )
					{
						...
						if ( from is SabretoothBearRiding ){ stuffed = 0x56A3; }
						else if ( from is BrownBear ){ stuffed = 0x569B; }
						else if ( from is SabreclawCub ){ stuffed = 0x569B; bearhue = 443; }
						else if ( from is BlackBear ){ stuffed = 0x569B; bearhue = 0xB3A; }
						else if ( from is GreatBear ){ stuffed = 0x569F; bearhue = 0xAC0; }
						else if ( from is KodiakBear ){ stuffed = 0x569F; bearhue = 0xAB1; }
						else if ( from is PolarBear ){ stuffed = 0x569F; }
						else if ( from is GrizzlyBearRiding ){ stuffed = 0x569D; }
						else if ( from is DireBear ){ stuffed = 0x569F; bearhue = 0x92B; }
						else if ( from is ElderBrownBear ){ stuffed = 0x56A3; }
						else if ( from is SabretoothBear ){ stuffed = 0x840; }
						else if ( from is ElderPolarBear ){ stuffed = 0x56A7; }
						else if ( from is ElderBlackBear ){ stuffed = 0x56A5; }
						else if ( from is CaveBear ){ stuffed = 0x6DE; }
						else if ( from is ElderBrownBearRiding ){ stuffed = 0x56A3; }
						else if ( from is SabretoothBearRiding ){ stuffed = 0x840; }
						else if ( from is ElderPolarBearRiding ){ stuffed = 0x56A7; }
						else if ( from is ElderBlackBearRiding ){ stuffed = 0x56A5; }
						else if ( from is CaveBearRiding ){ stuffed = 0x56A9; }
						else if ( from is LionRiding ){ stuffed = 0x56A1; }
						else if ( from is SnowLion ){ stuffed = 0x56A1; bearhue = 0xB4D; }
					...
					if ( reg.IsPartOf( "the Ancient Elven Mine" ) )
					{
						...
						{
							Item stone = new RareMetals( Utility.RandomMinMax( 1, 3 ), "silver stones" );
							c.DropItem(stone);
						}
					...
					if ( from is Pegasus || from is Placeron || from is PegasusRiding ){ if ( 75 < Utility.Random( 100 ) ){ c.DropItem( new PegasusFeather() ); } }
