...
//				list.Add( 1060660, "team\t{0}", m_Team );
		...
		public static void SpreadOut( Mobile m )
		{
			...
			if ( m is BaseVendor || m is Adventurers || m is Jedi )
			{
				...
				if ( m.X >= 0 && m.Y >= 0 && m.X <= 6 && m.Y <= 6 && m.Map == Map.Lodor ){ cleanUp = true; m.Location = Worlds.GetRandomLocation( "the Land of Lodoria", "land" ); m.WhisperHue = 911; }
				else if ( m.X >= 0 && m.Y >= 0 && m.X <= 6 && m.Y <= 6 && m.Map == Map.Sosaria ){ cleanUp = true; m.Location = Worlds.GetRandomLocation( "the Land of Sosaria", "land" ); m.WhisperHue = 911; }
				else if ( m.X >= 0 && m.Y >= 0 && m.X <= 6 && m.Y <= 6 && m.Map == Map.SerpentIsland ){ cleanUp = true; m.Location = Worlds.GetRandomLocation( "the Serpent Island", "land" ); m.WhisperHue = 911; }
				else if ( m.X >= 0 && m.Y >= 0 && m.X <= 6 && m.Y <= 6 && m.Map == Map.IslesDread ){ cleanUp = true; m.Location = Worlds.GetRandomLocation( "the Isles of Dread", "land" ); m.WhisperHue = 911; }
				else if ( m.X >= 1125 && m.Y >= 298 && m.X <= 1131 && m.Y <= 305 && m.Map == Map.SavagedEmpire ){ cleanUp = true; m.Location = Worlds.GetRandomLocation( "the Savaged Empire", "land" ); m.WhisperHue = 911; }
				else if ( m.X >= 5457 && m.Y >= 3300 && m.X <= 5459 && m.Y <= 3302 && m.Map == Map.Sosaria ){ cleanUp = true; m.Location = Worlds.GetRandomLocation( "the Land of Ambrosia", "land" ); m.WhisperHue = 911; }
				else if ( m.X >= 608 && m.Y >= 4090 && m.X <= 704 && m.Y <= 4096 && m.Map == Map.Sosaria ){ cleanUp = true; m.Location = Worlds.GetRandomLocation( "the Island of Umber Veil", "land" ); m.WhisperHue = 911; }
				else if ( m.X >= 6126 && m.Y >= 827 && m.X <= 6132 && m.Y <= 833 && m.Map == Map.Sosaria ){ cleanUp = true; m.Location = Worlds.GetRandomLocation( "the Bottle World of Kuldar", "land" ); m.WhisperHue = 911; }
				else if ( m.X == 4 && m.Y == 4 && m.Map == Map.Underworld ){ cleanUp = true; m.Location = Worlds.GetRandomLocation( "the Underworld", "land" ); m.WhisperHue = 911; }
	...
	public class SpawnGenerator
	{
		...
		public static void Parse( Mobile from, string filename )
		{
			string monster_path1 = Path.Combine( Core.BaseDirectory, "Data/Spawns" );
				if ( filename == "Spawns.map" ){ monster_path1 = Path.Combine( Core.BaseDirectory, "Info/Custom" ); }
