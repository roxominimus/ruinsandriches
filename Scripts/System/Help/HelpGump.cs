	...
	public class HelpGump : Gump
	{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID >= 200 && info.ButtonID <= 400 ) // MAGIC BARS OPEN AND CLOSE
			{
				...
				else if ( info.ButtonID == 268 ){ InvokeCommand( "knighttool1", from ); }
				else if ( info.ButtonID == 368 ){ InvokeCommand( "knightclose1", from ); }
				else if ( info.ButtonID == 269 ){ InvokeCommand( "knighttool2", from ); }
				else if ( info.ButtonID == 369 ){ InvokeCommand( "knightclose2", from ); }
				...
				else if ( info.ButtonID == 282 ){ InvokeCommand( "elementtool1", from ); }
				else if ( info.ButtonID == 382 ){ InvokeCommand( "elementclose1", from ); }
				else if ( info.ButtonID == 283 ){ InvokeCommand( "elementtool2", from ); }
				else if ( info.ButtonID == 383 ){ InvokeCommand( "elementclose2", from ); }
			}
			else
			{
		...
		public static string MyQuests( Mobile from )
        {
			...
			if ( PlayerSettings.GetBardsTaleQuest( from, "BardsTaleWin" ) && ((PlayerMobile)from).Profession != 1 ){ sQuests = sQuests + "-Defeated the evil wizard Mangar and escaped Skara Brae.<br><br>"; }
