...
namespace Server.Engines.Harvest
{
	public class Fishing : HarvestSystem
	{
		...
		public static double PoleSkill( Mobile m )
		{
			...
			if ( m.FindItemOnLayer( Layer.OneHanded ) != null && m.FindItemOnLayer( Layer.OneHanded ) is FishingPole ) { pole = m.FindItemOnLayer( Layer.OneHanded ); }
			else if ( m.FindItemOnLayer( Layer.FirstValid ) != null && m.FindItemOnLayer( Layer.FirstValid ) is FishingPole ) { pole = m.FindItemOnLayer( Layer.FirstValid ); }

			double bonus = 0.0;

			if ( pole != null ){ bonus = FishingPole.PoleSkill( pole ); }
		...
		public static double FishSkills( Mobile from, bool checkAll )
		{
			...
			if ( checkAll ){ skill = from.Skills[SkillName.Seafaring].Value; }
