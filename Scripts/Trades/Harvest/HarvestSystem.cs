	...
	public abstract class HarvestSystem
	{
		...
		public virtual void FinishHarvesting( Mobile from, Item tool, HarvestDefinition def, object toHarvest, object locked )
		{
			...
			if ( tool is FishingPole )
			{
				...
				{
					from.SendMessage("You will only get better at seafaring if you fish from a boat.");
					...
					else
					{
						if ( item.Stackable )
						{
							...
							if ( addUp )
							{
								...
								while ( skillCycle > 0 )
								{
									extra++;	if ( extra > Server.Misc.MyServerSettings.StatGainDelayNum() ){ extra = 1; }
							...
							if ( ( Worlds.IsExploringSeaAreas( from ) || reg.IsPartOf( "Shipwreck Grotto" ) || reg.IsPartOf( "Barnacled Cavern" ) ) && FindSeaLog && item is BaseLog )
							{
							...
							else if ( Worlds.GetMyWorld( from.Map, from.Location, from.X, from.Y ) == "the Underworld" && FindToughLog )
							{
							...
							if ( this == Mining.System && Worlds.GetMyWorld( from.Map, from.Location, from.X, from.Y ) == "the Savaged Empire" && from.Skills[SkillName.Mining].Value > Utility.RandomMinMax( 1, 500 ) )
							{
							...
							else if ( this == Mining.System && Worlds.GetMyWorld( from.Map, from.Location, from.X, from.Y ) == "the Island of Umber Veil" && from.Skills[SkillName.Mining].Value > Utility.RandomMinMax( 1, 500 ) )
							{
								Container pack = from.Backpack;
							...
							if ( tool is FishingPole && Server.Engines.Harvest.Fishing.IsNearHugeShipWreck( from ) && Server.Engines.Harvest.Fishing.FishSkills( from, true ) >= Utility.RandomMinMax( 1, 250 ) )
							{
								Server.Engines.Harvest.Fishing.FishUpFromMajorWreck( from );
							}
							else if ( tool is FishingPole && Server.Engines.Harvest.Fishing.IsNearSpaceCrash( from ) && Server.Engines.Harvest.Fishing.FishSkills( from, true ) >= Utility.RandomMinMax( 1, 250 ) )
							{
								Server.Engines.Harvest.Fishing.FishUpFromSpaceship( from );
							}
							else if ( tool is FishingPole && Server.Engines.Harvest.Fishing.IsNearUnderwaterRuins( from ) && Server.Engines.Harvest.Fishing.FishSkills( from, true ) >= Utility.RandomMinMax( 1, 250 ) )
							{
								Server.Engines.Harvest.Fishing.FishUpFromRuins( from );
							}
						}
						else if ( item is BlueBook || item is LoreBook || item is DDRelicBook || item is MyElementalSpellbook || item is MyNecromancerSpellbook || item is MySpellbook || item is MyNinjabook || item is MySamuraibook || item is MyPaladinbook || item is MySongbook || item is ArtifactManual )
						{
		...
		public virtual HarvestResource MutateResource( Mobile from, Item tool, HarvestDefinition def, Map map, Point3D loc, HarvestVein vein, HarvestResource primary, HarvestResource fallback )
		{
			...
			double skillValue = from.Skills[def.Skill].Value;
				if ( tool is FishingPole ){ skillValue = Server.Engines.Harvest.Fishing.FishSkills( from, true ); }
